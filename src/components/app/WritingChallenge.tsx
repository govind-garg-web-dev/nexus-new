...
export default function WritingChallenge({ challenge }: { challenge: Challenge }) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 lg:p-10">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-tech text-xs text-white/40">WRITING</span>
          </div>
      ...
      {/* Prompt */}
      <div className="rounded-2xl p-6 border border-white/5 mb-6" style={{ background: "rgba(255,255,255,0.02)" }}>
        <p className="font-tech text-xs font-semibold text-violet-400 mb-3 tracking-wider">PROMPT</p>
        ...
        <div className="mt-4 flex items-center gap-4">
          <span className="font-tech text-xs text-white/40">Word limit: {challenge.word_limit}</span>
          <span className="font-tech text-xs text-white/40">Time: {Math.round(challenge.time_limit_seconds / 60)} min</span>
          <span className="font-tech text-xs text-amber-400">Peer reviewed — badge after 2 approvals</span>
        </div>
      </div>
      ...
      <div className="relative mb-4">
        <textarea
          ...
          rows={14}
          disabled={!!status}
          className="w-full px-5 py-4 rounded-2xl border border-white/8 text-white placeholder-white/20 font-tech text-sm leading-relaxed resize-none focus:outline-none focus:border-violet-500/40 transition-colors disabled:opacity-60"
        ...
        <div className="absolute bottom-4 right-4 flex items-center gap-3">
          ...
          <span className={`font-tech text-xs ${wordCount > challenge.word_limit ? "text-red-400" : "text-white/40"}`}>
      ...
      <AnimatePresence>
        {status === "under_review" && (
          <motion.div
            ...
            className="mb-4 px-5 py-5 rounded-2xl border border-amber-500/25 bg-amber-500/5"
          >
            ...
            <p className="font-tech text-sm text-white/50">
              Two verified writers will review your response. Your badge is minted automatically once both approve.
              Check the <a href="/challenges/review" className="text-violet-400 hover:text-violet-300 underline">Review Queue</a> to see status.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
