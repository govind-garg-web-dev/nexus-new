...
const NPS_KEY     = "matchbatch_nps_completed";
const NPS_SHOWN   = "matchbatch_nps_last_shown";
...
export default function NpsPrompt({ userId }: { userId: string }) {
  ...
  useEffect(() => {
    ...
    const accountCreated = localStorage.getItem("matchbatch_created_at");
    if (accountCreated) {
        ...
        >
          {phase === "done" ? (
            <div className="p-6 text-center">
              ...
              <p className="font-tech text-xs text-white/50 mt-1">Your feedback helps shape MatchBatch.</p>
            </div>
          ) : (
            <div className="p-5">
              {/* Header */}
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="font-tech text-xs text-white/50 mt-0.5">
                    {phase === "rating"
                      ? "How likely are you to recommend MatchBatch to a friend at your campus?"
